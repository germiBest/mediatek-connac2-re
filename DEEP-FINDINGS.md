# MT7961 (connac2) WiFi MCU Firmware: Consolidated RE Findings

Target: MediaTek connac2 MT7961 WiFi MCU, Tensilica Xtensa LX core with custom vendor TIE (op0 = 0x4 / 0xE / 0xF). Artifacts: `region0.bin` RAM code @ `0x915000`, `region1.bin` rodata @ `0x02015c00`, `WIFI_ROM_MT7961_full.bin` 1 MB mask ROM @ `0x800000`. Driver cross-reference: in-tree `mt76` at `/home/germi/src/mt76`.

**Method caveat that governs everything below:** the Ghidra Xtensa decoder and capstone both desync on TIE opcodes, and the decompiler times out on every non-trivial handler. So byte-level field reads from the *middle* of a handler are not reliable. Three things that ARE ground truth were leaned on: (a) the dispatch tables are **data** in `region1.bin`, read byte-exact; (b) `mt76` driver structs are the wire layout the firmware parses, by definition; (c) a handful of **clean base-ISA ops** (immediate loads, range-compare gates, verified jump/call displacements) survive TIE desync and anchor specific layouts to specific firmware addresses. Each claim below is tagged **[proven]** (read from data/clean-ISA at a cited address) or **[inferred]** (driver layout or connac2-family cross-reference, not read out of this firmware byte-by-byte).
...
This is the strongest result: the RLM TLV (`tag 0x02`, handler `FUN_0091bfb4`) is the channel-grant input, and its layout is byte-identical to the firmware's internal channel struct because the handler `memcpy`s the body verbatim.
...
### 1.5 NEW vs already-had (track 1)

**Already had** (stated as verified anchors going in): BSS_INFO_UPDATE -> UNI dispatch @ `0x02022ce0` -> `FUN_009182ae`; the BSS_INFO TLV table @ `0x02018120` with tag2 RLM -> `FUN_0091bfb4` -> `FUN_009241dd`, plus tag1/4/5/16 handlers; `COMMAND-MAP.md` prior table map.

**NEW:** (a) the single combined EXT+UNI table insight (UNI = byte-continuation @ `0x02022de8`, arithmetically verified); (b) the per-byte RLM field naming with the `movi.n 0xc` / `blti a5,7` anchors and the `0x0091bfb4 -> 0x009241dd -> 0x00924e03` chain plus channel-mgr global `0x008f8420`; (c) the additional command entry points (ROC/PWR/EFUSE_CTRL cluster `0x009175b4`, FW_LOG `0x00917006`, PM/CHANNEL_SWITCH `0x00916f8a`); (d) the full 18-entry TLV table with the 5 firmware-internal tags identified; (e) driver-layout field tables for BASIC/BSS_COLOR/HE_BASIC/QBSS.

---

## 2. SDK / source availability

**Verdict: DEAD END for the firmware source, PARTIAL WIN for surrounding context.**

The MCU scheduler source (`cnm_open.c` / `getMinimumQuotaTime` / the `ENUM_CNM_QUOTA_CHINFO` role switch) is confirmed closed. Every firmware-unique symbol returns **0 hits** on GitHub code search and web search **[proven by negative search this session]**:
...
**Bottom line:** use gen4m `cnm.h` to name the channel-privilege protocol and the `Mcc*QuotaTimeInUs` config surface, then read the quota scheduler itself in Ghidra (region0 grant cluster ~`0x0094e4fa..0x0094fc04`, `getMinimumQuotaTime` consumer). It cannot be sourced.

### NEW vs already-had (track 2)

**Already had:** the symbol/string list itself (memory note on the MCC scheduler, quota knob names). **NEW:** the definitive negative-search result that closes the question (these exact symbols/paths = 0 public hits), and the gen4m proxy chain with concrete URLs, including the `MccDualStaAIS0/1QuotaTimeInUs` host config token as the nearest public analogue.
...
- USB device controller @ `0xe0000000` + `0xe0270000` (region3 IRAM). USB-side command handler in IRAM: EXT cid 0x16 -> `0xe027074e`. Endpoint strings "USB WiFi"/"USB WFSYS"/"USB_UDMA_WFSYS" @ `0x8172c0`/`0x817e60`/`0x817e6c`.
...
### 3.4 NEW vs already-had (track 3)

**Already had:** the ROM filename islands (`sys_efuse.c`, `hif.c`, `cnm*.c`, `top_pos.c`, etc.) and the EXT/UNI dispatch table addresses. **NEW:** (a) the eFuse command path resolved to handler addresses (`FUN_0091837e` / `FUN_009175b4`) plus the buffer-mode shadow mechanism and its strings; (b) the negative literal-pool finding that the ROM does NOT classic-kick the efuse controller; (c) the WFDMA ring-register map with `FUN_00818654` as the ring programmer, the USB IRAM command handler `0xe027074e`, and the HIF ISR symbol `0x02017238`; (d) VECBASE structure with panic stub `0x00803090` / assert printf `0x00800598`, the verified ROM mirror, the no-signature-gate download observation, and the WM entry `j 0x917405`.

### 3.5 Global reliability caveats
